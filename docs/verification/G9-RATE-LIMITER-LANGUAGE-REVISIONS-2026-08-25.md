...
Date: 2026-08-25  
Status: **complete**  
Scope: the five executable code rate-limiter revisions, the separate
PostgreSQL SQL family, and Lab editor coverage

## What changed

`task-family.rate-limiter` now has five runnable code revisions that share one
capability, Question Brain bindings, rubric, and learning brief while keeping
language-specific starter files and harnesses. The PostgreSQL implementation
is a separate `task-family.postgresql-rate-limiting` SQL family: it is a
database exercise with its own brief and rubric, not a sixth programming
language variant of the application limiter:
...
| Family | Revision | Sandbox profile | Editable file |
| --- | --- | --- | --- |
| `task-family.postgresql-rate-limiting` | `pg-rate-limiter-001@1` | PostgreSQL 17 | `solution.sql` |
...
- Runtime release: `runtime-task-release-2026-08-26-qb-d00a1493-g10`
- TaskFamily release: `task-family-release-2026-08-26-g10`
- Question Brain release: `question-release-d00a14931e607336`

The workspace launcher and Task Runtime Compose default now select the g10
manifest. Historical g8 and g9 releases remain immutable.
...
All five code revisions and the separate SQL revision were executed through
`POST /v1/runs` using the real Docker sandbox. Each returned HTTP 200 and all
four checks passed:
...
[
  ...
  {"taskId":"pg-rate-limiter-001","family":"task-family.postgresql-rate-limiting","http":200,"status":"pass","tests":["pass","pass","pass","pass"]}
]
```
